const TILE_SIZE=32;
let map={width:20,height:20,spawn:{x:0,y:0},tiles:[],objects:[],teleports:[]};
...
const col=['#000','#777','#0c0','#09f','#b000ff'];
...
ctx.fillStyle='yellow';for(const o of map.objects)ctx.fillRect(o.x*TILE_SIZE+8,o.y*TILE_SIZE+8,16,16);ctx.fillStyle='white';ctx.font='14px Arial';ctx.textAlign='center';if(map.teleports){for(const tp of map.teleports){ctx.fillText(tp.id.toUpperCase(),tp.x*TILE_SIZE+16,tp.y*TILE_SIZE+20);}}
...
else{map.tiles[y][x]=selectedTile;if(selectedTile===4){map.teleports??=[];let id=prompt('Teleport-ID (A)');if(id){map.teleports=map.teleports.filter(t=>!(t.x===x&&t.y===y));map.teleports.push({id:id.toLowerCase(),x,y});}}}drawMap();}
...
function importMap(f){let r=new FileReader();r.onload=()=>{map=JSON.parse(r.result);if(!map.spawn)map.spawn={x:0,y:0};if(!map.teleports)map.teleports=[];w.value=map.width;h.value=map.height;drawMap();};r.readAsText(f);}
createMap(20,20);
