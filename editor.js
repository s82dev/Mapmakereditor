...
function createMap(w,h){map={width:w,height:h,tiles:[],objects:[]};for(let y=0;y<h;y++){let r=[];for(let x=0;x<w;x++)r.push(0);map.tiles.push(r);}drawMap();}
...
function importMap(f){let r=new FileReader();r.onload=()=>{map=JSON.parse(r.result);w.value=map.width;h.value=map.height;drawMap();};r.readAsText(f);}
createMap(20,20);
